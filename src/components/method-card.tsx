...
export default function MethodCard({ method, mindsets, successLogs, isArchived = false }: MethodCardProps) {
  const [isExpanded, setIsExpanded] = useState(false)
  const { isOpen: isLogFormOpen, open: openLogForm, setIsOpen: setLogFormOpen } = useDialog()
  const mindsetTitle = mindsets.find((m) => m.id === method.mindsetId)?.title || "不明"
  ...
  const handleArchiveToggle = async () => {
    try {
      ...
      console.error(`Failed to ${isArchived ? "unarchive" : "archive"} method:`, error)
    }
  }
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex justify-between items-center mb-4">
          <div className="text-sm text-muted-foreground">成功ログ: {successLogs.length}件</div>
        </div>
      ...
      <SuccessLogForm methodId={method.id} isOpen={isLogFormOpen} onOpenChange={setLogFormOpen} />
    </Card>
  )
}
